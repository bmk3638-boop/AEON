...
질문 받을 때 이 순서로 참조:
1. CLAUDE.md (이 파일)
2. `wiki/hot.md` (최근 컨텍스트 캐시, ~500자)
3. `wiki/index.md` (전체 카탈로그)
4. `00_사업/서비스카탈로그.md`
5. `01_고객/ICP.md`
6. 해당 폴더의 관련 파일
...
볼트 내 wiki/ 폴더는 LLM이 유지하는 구조화된 지식 베이스입니다.

```
wiki/hot.md      → 먼저 읽기 (최근 컨텍스트 요약)
wiki/index.md    → 전체 페이지 카탈로그
wiki/entities/   → 사람·조직·프로젝트·도구
wiki/concepts/   → 개념·패턴·아키텍처
wiki/domains/    → 기능 영역별 기술 문서
wiki/decisions/  → 기술 결정 기록 (ADR)
wiki/questions/  → 저장된 Q&A
```

새 정보·결정이 생길 때마다 wiki 페이지 업데이트 또는 신규 생성.
wiki는 채팅이 아닌 **지식 자산**으로 축적됨.
